...
void Finger::Build(FWSceneIf* fwScene, PHSolidIf* gripDevice) {
	...
	sprDesc.spring = 700 * Vec3d(1, 1, 1);	//	0.5N/mm = 500N/m
	sprDesc.damper = sprDesc.spring * 0.06;
...
FingerGrip::FingerGrip() {
	fingers.resize(3); // for two finger
	//fingers.resize(2);  // for three finger
	for (int i = 0; i < fingers.size(); ++i) {
		Finger& finger = fingers[i];
		finger.index = i;
		finger.direction = Quaterniond::Rot(Rad(180), 'z') * Quaterniond::Rot(Rad(120 * i), 'z') * Vec3d(1, 0, 0);
		finger.position = Quaterniond::Rot(Rad(120 * i), 'z') * Vec3d(finger.maxLength, 0, 0); // for  three finger.
		//finger.position = Quaterniond::Rot(Rad(180 * i), 'z') * Vec3d(finger.maxLength, 0, 0);  // for two finger
	}
}
void FingerGrip::Build(FWSceneIf* fwScene) {
	...
	CDBoxDesc bd;
	bd.boxsize = 0.001 * Vec3d(2, 1, 0.5);
